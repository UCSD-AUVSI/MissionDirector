...
def ReconnectArduinoButtonAction():
	remotemsg = {"cmd":"arduino", "args":{"reconnect":" "}}
	fwdmsg = {"cmd":"planeobc:"}
	fwdmsg["args"] = {"message":json.dumps(remotemsg),"ip":planeOBCIPVar.get()}
	send_message_to_client(json.dumps(fwdmsg), ports.listenport_HumanOperator, mDIPAVar.get())
...
Button(middleFrame, text = "Reconnect Arduino", width = 16, command = ReconnectArduinoButtonAction).grid(row = 16, column = 0)
Button(middleFrame, text = "Disconnect Arduino", width = 16, command = DisconnectArduinoButtonAction).grid(row = 16, column = 1)
